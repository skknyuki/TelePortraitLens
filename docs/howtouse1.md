...
カメラオブジェクトの視線の先に**２つの四角い枠**表示されます。  
このカメラと枠は、自分自身からしか見ることが出来ません。  
![](images/how_Line.jpg)

## 基本的な仕様
レンズを起動させるとVRCハンドカメラ上の画面が  
TeleportraitLensの画面になってます。  
  
**Tele Portrait Lens**は３つのカメラを使用しています。
**前景（Fore）**・**メイン**・**背景（Back）**の３つを同時に撮影し  
それを合成しています。
  
表示されている**四角い枠組の中が**メインカメラの撮影範囲になります。  
...
2. [P]-[Focus]を選択すると、上下左右の4方向に入力できるパットが表示されます。  
...
5. Optional:[M]-[各カメラ]-[Color Correction]でカメラの色調補正などが出来ます。  
メインに映っている被写体だけを明るくしたり  
逆に背景を暗くしたりして、被写体を目立させる事も可能です。

6. 設定をリセットしたい場合には[Preset]の各種[Reset]を活用してください。  
押し間違い防止のために,{Lens}{Fix&Level}のResetボタンは  
長押ししないとResetされません。  
